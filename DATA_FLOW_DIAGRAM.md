...
- **Programming (adaptive scaffolding):** The `programming` skill is in `skills` with AI-generated `programming_mcq` rows. `GET /api/items/queue/programming` calls `ensureProgrammingQueueItems` in `backend/src/services/content-generator.js` (prompt `programming-generator.md`). Items include `metadata.structured_steps` for spoon-feed steps and an A–D MCQ in `prompt.text`. Grading reuses shared letter-MCQ logic (`gradeProgrammingMcqLocal`). Hints use `generateProgrammingHint` (`programming-hint.md`). After each graded attempt, `updateProgrammingScaffoldingAfterAttempt` in `backend/src/services/programming-adaptation.js` updates `student_tuning.programming_scaffolding` (1–5) from correctness, `hint_used`, and response time; `session_item_count_programming` controls session length (1–3).
