...
import { dispatchManualAgentRun, scheduleAgentCronJobs } from "../services/agent-scheduler.js";
...
const AGENT_NAMES = new Set(["calibration", "content", "frustration", "insight"]);
...
router.post("/actions/:id/approve", async (req, res, next) => {
  try {
    ...
    if (row.action_type !== "propose_tuning_change") {
    ...
    if (typeof sid !== "string" || !isUuid(sid) || typeof param !== "string" || !Number.isFinite(Number(proposed))) {
